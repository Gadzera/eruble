...
async function seed() {
  ...
  if (existing.length > 0) {
    console.log("DB already seeded — delete data/orca.db to reset.");
    return;
  }
  console.log("Seeding…");
  ...
  const ops: OpRow[] = [];
  let balance = 2_000_000_00; // 2M стартовый баланс в центах

  // Автоматическое пополнение если недостаточно средств
  ...
  const qrProducts = [
    "трансформаторное оборудование ТМ-1000/10",
    "кабель силовой ВВГ 3×185, 4×150",
    "щиты управления ГРЩ и ВРУ",
    "устройства плавного пуска АТС",
    "частотные преобразователи серии G",
    "коммутационное оборудование КРУ",
    "распределительные устройства РУ-10кВ",
    "контрольно-измерительные приборы",
    "силовые трансформаторы ТМГ",
    "электрические щитки и распределители",
    "кабельные лотки и короба",
    "реле защиты и автоматики",
  ];
  let qrInvNum = 312;
  ...
  let supplierPurposeIdx: Record<number, number> = {};
  let ipPurposeIdx: Record<number, number> = {};
  ...
  function statusForDay(dAgo: number, opIdx: number): {
    sd: string; sb: string | null; sp: string | null; se: string | null;
    submittedAt: number | null; executedAt: number | null;
  } {
    const t = at(dAgo, 12);
  ...
  // Массивы циклических значений
  const clients    = [0, 1, 2, 3, 4, 5, 6, 7]; // индексы покупателей
  ...
  // ── 90-дневный цикл операций ───────────────────────────────────────────────
  for (let d = 90; d >= 0; d--) {
    // Пополнение кошелька каждые 6 дней
    if (d % 6 === 0) {
      addCashIn(d, cinAmounts[ci++ % cinAmounts.length]);
    }

    // Вывод на р/с каждые 18 дней
    if (d % 18 === 0 && d < 90) {
      addCashOut(d, 350 + (d % 3) * 50);
    }

    // Входящий платёж от клиента каждые 2 дня
    if (d % 2 === 0) {
      addIncoming(d, clients[qi % clients.length], qrAmounts[qi % qrAmounts.length], 10);
      qi++;
    }

    // Дополнительный входящий каждые 7 дней (крупный контракт)
    if (d % 7 === 0) {
      addIncoming(d, clients[(qi + 3) % clients.length], qrAmounts[(qi + 5) % qrAmounts.length] + 1000, 11);
      qi++;
    }

    // Платёж поставщику — каждый день
    {
      const cpIdx = suppliers[si % suppliers.length];
      const amt = supAmounts[si % supAmounts.length];
      const cnum = contractCounter++;
      addB2B(d, cpIdx, amt, getSupplierPurpose(cpIdx, cnum), 13, opCnt++);
      si++;
    }

    // Второй поставщик каждые 2 дня
    if (d % 2 === 1) {
      const cpIdx = suppliers[(si + 3) % suppliers.length];
      const amt = supAmounts[(si + 7) % supAmounts.length];
      const cnum = contractCounter++;
      addB2B(d, cpIdx, amt, getSupplierPurpose(cpIdx, cnum), 15, opCnt++);
      si++;
    }

    // ИП каждые 3 дня
    if (d % 3 === 0) {
      const cpIdx = ipCps[ii % ipCps.length];
      const amt = ipAmounts[ii % ipAmounts.length];
      const month = d <= 30 ? "апрель" : d <= 60 ? "март" : "февраль";
      addB2B(d, cpIdx, amt, `${getIPPurpose(cpIdx)}, ${month} 2026`, 14, opCnt++, uPay);
      ii++;
    }

    // Аренда офиса — 1-го числа каждого месяца (раз в ~30 дней)
    if (d === 88 || d === 58 || d === 28) {
      addB2B(d, 21, 320, "Аренда офисного помещения по договору №АП-2023/08", 9, opCnt++, uCA);
    }

    // Ростелеком — начало месяца
    if (d === 87 || d === 57 || d === 27) {
      addB2B(d, 20, 48, "Услуги связи по договору №РТ-2026/01", 9, opCnt++, uCA);
    }

    // 1С — ИТС раз в месяц
    if (d === 86 || d === 56 || d === 26) {
      addB2B(d, 23, 75, "Сопровождение 1С:ERP по договору №ИТС-2026/01", 9, opCnt++, uCA);
    }

    // Погашение займа — ежемесячно (основной долг + проценты)
    if (d === 85 || d === 55 || d === 25) {
  ...
  // ── Черновики и ожидающие (последние 3 дня) ──────────────────────────────
  const draftSpecs = [
  ...
  // Дополнительные входящие крупные контракты
  addIncoming(5,  0, 2100, 9);  // Мосэнерго крупный контракт
  addIncoming(15, 2, 1600, 9);  // Россети крупный
  addIncoming(35, 1, 2500, 9);  // УралСталь крупный
  addIncoming(65, 5, 1900, 9);  // Северсталь крупный
  addIncoming(80, 3, 1500, 9);  // СтройПром крупный

  db.insert(schema.operations).values(ops).run();

  // ── Баланс кошелька — консистентный с операциями ─────────────────────────
  ...
  const walletMain = {
    id: id("wlt"), orgId: orgMain.id, externalRef: "DR000000000001",
    balanceCents: OPENING_CENTS + netCents,
    blockedCents: 0, status: "ACTIVE", lastSyncAt: minAgo(2), createdAt: daysAgo(400),
  };
  const walletBeta = {
    id: id("wlt"), orgId: orgBeta.id, externalRef: "DR000000000045",
    balanceCents: 890_000_00, blockedCents: 0, status: "ACTIVE", lastSyncAt: minAgo(15), createdAt: daysAgo(300),
  };
  db.insert(schema.wallets).values([walletMain, walletBeta]).run();
  ...
  for (let i = 0; i < regPayroll.rowsTotal; i++) {
    ...
    items.push({
      id: id("ri"), registryId: regPayroll.id, rowNumber: i + 1,
      recipientInn: null, recipientName: `Сотрудник №${2000 + i}`,
      recipientDrRef: `DR${String(3000 + i).padStart(12, "0")}`,
      amountCents: (60 + (i * 7) % 100) * 1_000_00,
      purpose: "Заработная плата за апрель 2026",
      status: valid ? "EXECUTED" : "INVALID",
      errorCode: valid ? null : "INVALID_DR_ACCOUNT",
      errorMessage: valid ? null : "Не найден ЦР-счёт получателя", operationId: null,
    });
  }
  for (let i = 0; i < regVendor.rowsTotal; i++) {
    const cpObj = counterparties[i % 13];
    items.push({
      id: id("ri"), registryId: regVendor.id, rowNumber: i + 1,
      recipientInn: cpObj.inn, recipientName: cpObj.name, recipientDrRef: cpObj.drAccountRef,
      amountCents: (80 + (i * 17) % 300) * 1_000_00,
      purpose: `Оплата по договору поставки №${200 + i}`, status: "VALID",
      errorCode: null, errorMessage: null, operationId: null,
    });
  }
  for (let i = 0; i < regTax.rowsTotal; i++) {
    const valid = i < regTax.rowsValid;
    items.push({
      id: id("ri"), registryId: regTax.id, rowNumber: i + 1,
      recipientInn: "7707030001", recipientName: "ФНС России", recipientDrRef: "DR999000000001",
      amountCents: (100 + (i * 29) % 200) * 1_000_00,
      purpose: "Уплата налога Q1 2026, КБК 18210101011011000110",
      status: valid ? "VALID" : "INVALID",
      errorCode: valid ? null : "MISSING_KBK", errorMessage: valid ? null : "Не указан КБК", operationId: null,
    });
  }
  ...
  for (let i = 0; i < 120; i++) {
    ...
    auditEvents.push({
      id: id("aud"), orgId: orgMain.id, actorId: u.id, actorName: u.name,
      action: a.action, objectType: a.obj, objectId: nanoid(8),
      ip: `10.0.${(i * 7) % 200}.${(i * 13) % 250}`,
      userAgent: "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Orca/1.0",
      payloadJson: null, severity: a.sev, createdAt: minAgo(i * 15 + (i % 5) * 3),
    });
  }
  ...
  const totalBalance = (OPENING_CENTS + netCents) / 100; // ₽
  console.log(`Seed OK: 2 orgs, 6 users, 3 banks, ${counterparties.length} cps, ${ops.length} ops, 3 registries, ${items.length} items, ${approvals.length} approvals, 120 audit, 4 notifications.`);
  console.log(`Wallet: ${(totalBalance / 1_000_000).toFixed(1)} млн ₽`);
}
